...
def test_raft_log_commit_and_revert():
    ...
    # Add entries
    raft_log.entries.append(LogEntry(term=1, command={"type": "NODE_JOIN", "node_id": "node2"}))
    raft_log.entries.append(LogEntry(term=1, command={"type": "ASSIGN_TASK", "task_id": "task1", "node_id": "node2"}))
...
def test_raft_log_apply_entry_node_fails():
    ...
    raft_log.apply_entry({"type": "ASSIGN_TASK", "task_id": "task1", "node_id": "node2"})
    assert raft_log.tasks["task1"] == "node2"
    assert "task1" in raft_log.uncompleted_tasks
